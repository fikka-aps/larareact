        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 text-gray-700">
          ...
          >
            {data.latestProgram && (
              <div>
                {/* <img
                  src={data.latestProgram.image_url}
                  className="w-[240px] mx-auto"
                /> */}
                <h3 className="font-bold text-xl mt-3 mb-3">
                  {data.latestProgram.title}
                </h3>
                <div className="flex justify-between text-sm mb-1">
                  <div>Create Date:</div>
                  <div>{data.latestProgram.created_at}</div>
                </div>
                <div className="flex justify-between text-sm mb-1">
                  <div>Expire Date:</div>
                  <div>{data.latestProgram.expire_date}</div>
                </div>
                <div className="flex justify-between text-sm mb-1">
                  <div>Status:</div>
                  <div>{data.latestProgram.status ? "Active" : "Draft"}</div>
                </div>
                <div className="flex justify-between text-sm mb-1">
                  <div>Questions:</div>
                  <div>{data.latestProgram.questions}</div>
                </div>
                <div className="flex justify-between text-sm mb-3">
                  <div>Answers:</div>
                  <div>{data.latestProgram.answers}</div>
                </div>
                <div className="flex justify-between">
                  <TButton to={`/programs/${data.latestProgram.id}`} link>
